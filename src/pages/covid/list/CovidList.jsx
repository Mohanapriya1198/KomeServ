...
import { covidData } from "../../../DummyData";
...
const CovidList = () => {
  ...
  useEffect(() => {
    if (searchData.search) {
      const newData = [];
      const filterData = covidData.filter((d) => {
        return (
      ...
      const Data = covidData.map((d, index) => {
        return { sn: index + 1, ...d };
      });
      setData(Data);
    }
  ...
  const viewHandler = (data) => {
    history.push({ pathname: "/covid/details/" + data.id, state: data });
  };

  const editHandler = (data) => {
    history.push({ pathname: "/covid/edit/" + data.id, state: data });
  };
  ...
  return (
    <Container className="covidList">
      <Table tableContainer="79.5vh" tableHeight="72vh">
        <div className="title">
          <span>Covid Measures List</span>
          <Search>
            <SearchIconWrapper>
              <SearchIcon />
            </SearchIconWrapper>
